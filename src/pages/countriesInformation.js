import {
  countriesTable,
  countrySelectedTable,
  thisCountryNotFound,
  loading,
} from "../view/countriesInformation.html.js";
  ...
  try {
    ...
    if (countries.includes(country)) {
      if (country.includes(" ")) {
        ...
        const countrySelectedVaccineData = await fetchVaccinatedCountry(
          longCountryName,
        );
        return countrySelectedTable(
          countrySelectedData,
          countrySelectedVaccineData,
        );
      } else {
...
const toCapitalCase = (country) => {
  ...
  if (countryStructure.length === 1) {
    return (
      countryStructure[0].toLowerCase().charAt(0).toUpperCase() +
      countryStructure[0].slice(1).toLowerCase()
    );
  } else if (countryStructure.length === 2) {
    const countryOfTwoWord = `${
      countryStructure[0].toLowerCase().charAt(0).toUpperCase() +
      countryStructure[0].slice(1).toLowerCase()
    } ${
      countryStructure[1].toLowerCase().charAt(0).toUpperCase() +
      countryStructure[1].slice(1).toLowerCase()
    }`;
    return countryOfTwoWord;
  } else {
    const countryOfThreeWord = `${
      countryStructure[0].toLowerCase().charAt(0).toUpperCase() +
      countryStructure[0].slice(1).toLowerCase()
    } ${countryStructure[1].toLowerCase()} ${
      countryStructure[2].toLowerCase().charAt(0).toUpperCase() +
      countryStructure[2].slice(1).toLowerCase()
    }`;
